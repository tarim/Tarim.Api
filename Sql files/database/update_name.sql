CREATE DEFINER=`root`@`localhost` PROCEDURE `UPDATE_NAME`(IN id_in INT,IN name_ug_in VARCHAR(64),IN name_latin_in VARCHAR(64),IN origin_in ENUM('Uyghur', 'Arabic', 'Persian', 'Other'),
...
origin=coalesce(origin_in,origin),gender=coalesce(gender_in,gender),
related_name=coalesce(related_name_in,related_name),is_surname=coalesce(is_surname_in,is_surname),
`description`=coalesce(description_in,`description`) WHERE recid=id_in;
END